...
```mermaid
flowchart TD
    subgraph CARD["1. Card Generation Flow"]
        A1["Select Category"] --> A2["Shuffle Words"] --> A3["Pick 24 Words"] --> A4["Render Card"]
    end

    subgraph SPEECH["2. Speech Recognition Flow"]
        B1["Audio Input"] --> B2["Speech API"] --> B3["Transcript Text"] --> B4["Word Detection"]
        B4 --> B5["Match Found?"] --> B6["Fill Square"] --> B7["Update UI"]
    end

    subgraph WIN["3. Win Detection Flow"]
        C1["Square Filled"] --> C2["Check Lines"] --> C3["BINGO? Yes!"] --> C4["Celebrate & Share"]
    end
```
